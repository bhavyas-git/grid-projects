...
# -----------------------------
# PREDICTION HISTORY (for bias monitoring)
# -----------------------------
prediction_history = []
...
model.load_state_dict(
    torch.load("../notebooks/checkpoints/mobilenet_v3_small_best.pth", map_location=device)
)
...
def predict_emotion(image):
    ...
    if not face_detected:
        ...
        fairness = "⚠️ No face detected — prediction may be unreliable"
    ...
    if len(prediction_history) > 10:
        ...
        ratio = prediction_history.count(most_common) / len(prediction_history)

        if ratio > 0.70:

            fairness += f"\n⚠️ Model frequently predicts '{most_common}' — possible dataset bias"
